...
export default function MapView({ entities, onSelectEntity, onAddEntity, onEditEntity, onDeleteEntity }: MapViewProps) {
  ...
  return (
    <div className="h-full flex gap-4 p-4">
      {/* Left Panel - Entity List (matches EntityList component) */}
        ...
        <div className="px-3 py-3 border-b border-gray-200">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-gray-500">
              {provinceFurnishers.length} {provinceFurnishers.length === 1 ? 'entity' : 'entities'}
              {(searchQuery || selectedProvince) && entities.length !== provinceFurnishers.length && (
          ...
          {selectedProvince && (
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 px-2 py-0.5 text-xs bg-blue-50 text-blue-700 rounded">
                {getRegionName(selectedProvince)}
                <button
